...
// Добавляет запись (new_scan) в файл (file) на место (new_place)
...
// Удаляет все пустые записи из файла (file)
...
int main()
{
  ...
  printf("Where to add the record: ");
  ...
  // Сжимаем записи в файле (удаляя пустые) и выводим всю информацию из файла в консоль
